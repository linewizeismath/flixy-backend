...
#error{position:fixed;left:12px;right:12px;bottom:12px;padding:10px 12px;background:rgba(180,0,0,.92);color:#fff;font-family:Arial,sans-serif;font-size:13px;border-radius:4px;display:none;word-break:break-word}
</style>
</head>
<body>
...
player.addEventListener("error",()=>{
    ...
    if(error) message += " MediaError code: " + error.code;
...
`);
        ...
        console.error("FLIXY PLAYBACK ERROR:", error);
        return res.status(500).send(
            `Flixy playback error: ${escapeHtml(error?.message || String(error))}`
        );
    }
});
